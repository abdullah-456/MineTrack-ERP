...
export default function BenchFormPage() {
  ...
  const submit = async (ev) => {
    ...
    if (!form.bench_number?.trim()) {
      error(`${t('required') || 'Required'}: ${t('benchNumber')}`);
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...
        bench_number: form.bench_number,
  ...
  return (
    <div className="space-y-6 w-full" dir={isRTL ? 'rtl' : 'ltr'}>
      <PageHeader
        ...
        subtitle={t('benchesSub') || 'Terraced level within a pit'}
        action={
      ...
      <form onSubmit={submit} className="space-y-4">
        <FormSection title={t('mineBasicInfo') || 'Basic Info'}>
          ...
          <div>
            <FormLabel required>{t('benchNumber')}</FormLabel>
            <input className="input" required value={form.bench_number} onChange={setF('bench_number')} />
          </div>
          <div>
