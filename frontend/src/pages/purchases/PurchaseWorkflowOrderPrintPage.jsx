...
export default function PurchaseWorkflowOrderPrintPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const { data } = await api.get(`/purchase-workflow/orders/${id}`, { params });
  ...
  return <PurchaseWorkflowOrderDocument order={order} showPrintBar />;
}
