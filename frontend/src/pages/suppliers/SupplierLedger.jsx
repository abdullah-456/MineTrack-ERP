        ...
        <Modal title={t('recordPayment') || 'Record Payment'} onClose={() => setModal(null)}>
          <form onSubmit={submitPayment} className="space-y-3">
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              {t('currentPayable') || 'Payable'}: {formatPKR(summary.current_payable, lang)}
              {parseFloat(summary.credit_balance) > 0 && (
                <> · {t('creditBalance') || 'Prepaid credit'}: {formatPKR(summary.credit_balance, lang)}</>
              )}
            </p>
            <div>
              ...
              {parseFloat(paymentForm.amount || 0) > parseFloat(summary.current_payable || 0) && (
